...
PAGES.pagos = {
  ...
  datafono: function (prefilledData = {}) {
    ...
    container.innerHTML = html`
      ...
      >
        ...
        <div
          style="margin: 0 auto;max-width: 435px;background: white;padding: 20px;border-radius: 10px;margin-bottom: 20px;"
        >
          <h2 style="text-align: center; color: #333; margin-top: 0;">
            Paso <span id="stepIndicator">1</span> de 3
          </h2>

          <!-- Step 1: Retailer Input -->
          <div id="step1">
            <fieldset style="border: 2px solid #667eea; border-radius: 8px; padding: 15px;">
              <legend style="color: #667eea; font-weight: bold;">
                1. Información de Transacción
              </legend>
              ...
              <label style="display: block; margin-bottom: 15px;">
                ...
                <select
                  id="${field_tipo}"
                  style="background: #fff; width: 100%; padding: 10px; font-size: 16px; border: 2px solid #ddd; border-radius: 5px;"
                >
          ...
          <div id="step2" style="display: none;">
            <fieldset style="border: 2px solid #667eea; border-radius: 8px; padding: 15px;">
              ...
              <div style="text-align: center; margin-bottom: 20px;">
                ...
                <div
                  style="font-size: 48px; font-weight: bold; color: #667eea; margin: 10px 0;"
                  id="step2Amount"
                >
                  0.00€
                </div>
          ...
          <div id="step3" style="display: none;">
            <fieldset style="border: 2px solid #667eea; border-radius: 8px; padding: 15px;">
              ...
              >
                <div style="margin-bottom: 10px;"><b>Tipo:</b> <span id="confirmTipo"></span></div>
              ...
              <div
                style="text-align: center; padding: 20px; background: #fff3cd; border-radius: 8px; border: 2px solid #ffc107;"
              >
        ...
        <div
          id="buttonContainerFinal"
          style="display: none; grid-template-columns: repeat(2, 1fr); gap: 10px;"
        >
          <button
            id="${btn_back}"
            style="padding: 20px; font-size: 18px; font-weight: bold; 
    ...
    document.getElementById(btn_confirm).onclick = () => {
      if (currentStep === 1) {
        ...
        // 🔒 VALIDACIÓN CORREGIDA DE SALDO INSUFICIENTE (ANTES DEL PASO 3)
        if ((tipo === 'Gasto' || tipo === 'Transferencia') && metodo === 'Tarjeta') {
          ...
          if (currentBalance < monto) {
            alert(
              `❌ Saldo insuficiente\n\nSaldo actual: ${currentBalance.toFixed(2)}€\nMonto requerido: ${monto.toFixed(2)}€`
            );
            return; // ⛔ NO pasar al paso 3
          }
        }
    ...
    function createPromoBonusTransaction(personaId, bonusAmount, originalAmount) {
      ...
      if (persona) {
        ...
        persona.Monedero_Balance = fixfloat(newBalance);
        DB.put('personas', personaId, persona).catch((e) => {
          console.warn('DB.put error', e);
        });
      }

      // Save bonus transaction
      DB.put('pagos', bonusTicketId, bonusData).catch((e) => {
        console.warn('DB.put error', e);
      });
    ...
    function handleSuperCafePayment(transactionData) {
      ...
      DB.put('personas', transactionData.Persona, persona).catch((e) => {
        console.warn('DB.put error', e);
      });
    }
    ...
    const config = [
      ...
      },
      {
        ...
        template: (data, element) => {
          ...
          element.innerHTML = html`<span
            style="background: ${color}; color: white; padding: 5px 10px; border-radius: 5px; font-weight: bold;"
            >${icon} ${tipo}</span
          >`;
        },
        default: '',
      },
      {
        ...
        template: (data, element) => {
          ...
          element.innerHTML = html`<span
            style="font-size: 20px; font-weight: bold; color: ${color};"
            >${monto.toFixed(2)}€</span
          >`;
        },
        default: '0.00€',
      },
      ...
      },
      {
        ...
        template: (data, element) => {
          ...
          element.innerHTML = html`<span
            style="background: ${color}; color: white; padding: 5px 10px; border-radius: 5px;"
            >${estado}</span
          >`;
        },
        default: 'Pendiente',
  ...
  __editTransaction: function (transactionId) {
    ...
    document.getElementById(btn_save).onclick = () => {
      ...
      document.getElementById('actionStatus').style.display = 'block';
      DB.put('pagos', transactionId, updatedData)
        .then(() => {
